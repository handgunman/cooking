...
2. **Версия** — последняя LTS (см п 0.3).
...
`GRANT SELECT, INSERT, ALTER TABLE, ALTER VIEW, CREATE TABLE, CREATE VIEW, CREATE DICTIONARY, DROP TABLE, DROP VIEW, TRUNCATE, dictGet, OPTIMIZE ON <имя базы>.* TO <имя пользователя>`.
...
дашборда, то это будет много</sub>
