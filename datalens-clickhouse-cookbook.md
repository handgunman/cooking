...
В этом кукбуке вы спроектируете и подготовите данные в ClickHouse для аналитики в DataLens: создадите базовую таблицу, настроите её под типовые BI-запросы, при необходимости добавите дополнительные структуры для ускорения запросов и подготовите производные данные для аналитики. Затем вы сможете подключить подготовленные данные к DataLens и оценить BI-нагрузку на ClickHouse.

Кукбук не обязательно проходить линейно. Сначала вы создадите базовую таблицу и проверите её на типовых запросах. Дальнейшие шаги зависят от задачи: для запросов, которые плохо обслуживает основной `ORDER BY`, рассмотрите projections; для отдельных сценариев фильтрации — skip-индексы; для работы со справочными данными — JOIN и словари; для производных показателей и предварительных расчётов — представления и материализованные представления.
...
- 0. Развернуть ClickHouse
  ...
  - 0.4 База данных и права пользователя
...
- 4. Использование таблиц - JOIN и словари
...
тестовых операций с ресурсами не менее 8CPU/32RAM, переходите к пункту 0.3.

### 0.1 Managed Service for ClickHouse

Подходит, если нет навыков самостоятельного развёртывания и администрирования
либо самой цели держать и обслуживать свою инфраструктуру. Глубоко вникать в настройки не
...
2. **Версия** — последняя LTS (см п 0.3). В данном материале используется 26.3.17.
...
5. **Хосты** — один хост для пилота и типовых задач. При требовании повышенной
   ...
   ClickHouse Keeper (на отдельном хосте для нагруженных систем).
6. **Настройки СУБД и пользователь** — задать имя и пароль пользователя, имя базы
   данных и выбрать метод управления пользователями и базой. Для BI завести
   отдельного пользователя, только на чтение нужной БД, можно добавить позже.
8. **Сервисные настройки** — включить доступ из DataLens и доступ из WebSQL.
9. **Настройки пользователей по умолчанию** - на данные момент включение
   анализатора (enableAnalyzer) вынесено в эту группу настроек. Включите её.
...
Внимание: анализатор должен быть включен `SELECT getSetting('enable_analyzer')`. 
Если настройка выключена для всего сервера, вы можете включить его для отдельного 
пользователя или профиля `ALTER USER user MODIFY SETTING enable_analyzer = 1`.
...
- Брать линию LTS, с релиза которой прошло 2-3 месяца — за это время
  устраняются основные проблемы релиза. То есть не 26.3.1, а 26.3.17 в июле 2026.
...
Для целей данного кукбука рационально будет создать отдельную базу данных и 
пользователя. В собственной установке и если выбран режим управления пользователями 
через SQL в Managed Service, пользователю нужно выдать права:
`GRANT SELECT, INSERT, ALTER TABLE, ALTER VIEW, CREATE TABLE, CREATE VIEW, CREATE DICTIONARY, DROP TABLE, DROP VIEW, TRUNCATE, dictGet, OPTIMIZE ON <имя базы>.* TO <имя пользователя>`. В Managed Service 
настройка более грубая.

---

## 1. Проектирование и подготовка таблиц

Производительность аналитических запросов, а значит и скорость загрузки и обновления 
чартов и дашбордов, сильно зависят от структуры хранения и витрин. Clickhouse 
значительно отличается от традиционных реляционный СУБД, вроде Oracle и PostgreSQL 
и далее рассмотрим основные моменты, которые необходимо учесть при проектировании.

### 1.1 Как будем готовить данные

Сначала создадим базовую широкую таблицу, выберем ключ сортировки и партиционирование, загрузим тестовые данные и проверим структуру на типовых аналитических запросах.

Базовой структуры может быть достаточно: если типовые BI-запросы выполняются с достаточной производительностью, дополнительная оптимизация не обязательна и таблицу сразу можно использовать как источник данных для DataLens. Если базовая структура не справляется с конкретными сценариями, переходите к дополнительным настройкам:

- **раздел 2:** если основной `ORDER BY` плохо обслуживает важные сценарии запросов, рассмотрите projections;
- **раздел 3:** если нужно дополнительно сокращать объем данных при фильтрации и распределение значений позволяет отбрасывать целые гранулы — skip-индексы;
- **раздел 4:** если для аналитики нужны данные из отдельных справочников — JOIN или dictionaries;
- **разделы 5–6:** если нужны производные показатели — представления, а если результаты тяжёлых или повторяющихся расчётов выгоднее готовить заранее — материализованные представления.

### 1.2 Где выполнять работы
...
- **WebSQL в Yandex Cloud** — те же данные доступны во вкладке метаданных.

Дашборд как измерительный прибор не годится: в нём не видно, сколько строк
реально прочитано и какой план был выбран. Но в дальнейшем из инспектора чартов 
можно брать реальные запросы, созданные BI, и проверять их работу в клиенте или 
WebSQL. Использовать полученные данные для донастройки витрин.

### 1.3 Широкая денормализованная таблица

> Если вы уже знакомы с базовыми принципами устройства таблиц в ClickHouse, можете пропустить теоретическую часть и перейти к разделу 1.6.
...
- Меняющаяся семантика справочных данных иногда не позволяет внести их в широкую
  ...
  вставки. Если справочные значения переименовывают, переклассифицируют или
...
- Массивы и JSON бывают удобны (переменный набор атрибутов, параметры событий,
  ...
  вычисляемых полей на распаковку. Следует использовать точечно, а не как способ
  не проектировать схему.
...
— потому что содержит по одной записи - значение первичного ключа из первой строки 
каждой гранулы (по умолчанию гранула содержит 8192 строк).
...
- Один `ORDER BY` не обслужит все сценарии. Если сценарии конфликтуют — это
  повод не на компромисс, а на проекцию (раздел 2) или отдельную таблицу.
...
- быстрый поиск по точному времени или конкретному id — точечная выборка, а не
  аналитика по срезам.

В BI такие случаи встречаются, но они не типовые. Не проектируйте под них
таблицу, которая обслуживает дашборды.

### 1.6 Партиционирование

Партиционирование это механизм управления данными, а не инструмент оптимизации 
запросов: перенос и отсоединение партиций, удаление старых данных, обработка 
по партициям, бэкапы. Партиционирование дробит данные — ClickHouse не мёрджит 
parts между партициями, что. порой, приводит к большему числу parts и замедлению 
запросов, охватывающих несколько партиций.
...
допустимо сделать партицию по полю, не входящему в `ORDER BY`, например даты 
с округлением, тогда патриция будет помогать с фильтрацией по этому полю.
...
### 1.7 Практика: создание таблицы и наполнение
...
### 1.8 Практика: два запроса для сравнения
...
Базовая таблица из раздела 1 оптимизирована под запросы с фильтрацией по `merchant_category` и `country`.
В этом разделе рассмотрим, как использовать projections для других сценариев запросов и проверить их влияние на объём чтения.

Для многих BI систем с небольшим, по меркам Clickhouse, объемом данных, 
продуманной структуры `ORDER BY` и `PRIMARY KEY` может быть достаточно. Но 
если вариативность фильтрации и объемы данных велики, то разреженных индексов 
и плюсов от методов хранения и сжатия может оказаться недостаточно.

> <sub>Много - это сколько? Предлагаемые сценарии реализуются на хосте 8CPU/32RAM. 
> После всех манипуляций таблица в 50 миллионов записей будет иметь размер порядка 
> 2.8 Гб в несжатом и 1.5 Гб в сжатом виде. Большинство запросов будет выполняться 
> доли секунд, так что можно сделать и 100 миллионов записей для большей 
> контрастности. И лишь при выборке с несколькими оконными функциями, время 
> выполнения превысит 5с. Но в реальности при построении дашборда в БД уходит сразу 
> несколько запросов. Каждый чарт и каждый селектор - это запрос, которому нужны 
> ресурсы. Дополнительно, не всегда получается делать эффективные структуры и 
> запросы, если соединяется 2-3 таблицы всего на 10 миллионов записей каждая, если 
> есть самосоединения больших таблиц, 7-8 уровней подзапросов, сомнительные оконные 
> функции, лишние сортировки и т.п. и все это помноженное на одновременные запросы 
> дашборда, то это будет много. Вычисляемые поля в Datalens добавляют нагрузки на
> CPU, уровни агрегации (LOD) реализуются через подзапросы, безопасные конструкции
> условий отбора усложняют задачу построения плана анализатору, все это умножается
> на одновременность запросов и снова может получиться много.</sub>

Проекция — дополнительный скрытый набор данных внутри той же таблицы,
поддерживаемый автоматически при вставке. Фактически проекция — это скрытая 
копия данных внутри каждого парта, со своим порядком строк, своим первичным 
индексом и, при необходимости, инкрементально пересчитываемыми агрегатами. 
Физически - поддиректория внутри директории парта, а не отдельный парт.
...
- **Индексоподобная** (`_part_offset`). Хранится только ключ сортировки
  ...
  С 26.1 доступен компактный синтаксис
...
Проекция с предагрегацией используется и для запросов с `DISTINCT`: если
...
таблице в сотни миллионов строк это несколько сканирований на каждое открытие и 
сама операция DISTINCT достаточно дорогая, в глазах пользователя выглядит как 
«дашборд грузится», хотя сами графики могут быть быстрыми. Особенно заметно 
на дашбордах с обилием селекторов, построенных на представлениях.
...
- Настройка `deduplicate_merge_projection_mode` определяет поведение
...
- По умолчанию лёгкие удаления на таблицах с проекциями завершаются ошибкой;
  поведение настраивается через `lightweight_mutation_projection_mode`:
  `throw`, `drop`, `rebuild`.
...
SELECT
    ...
    risk_score
FROM financial_transactions
WHERE (risk_score > 90) AND (amount > 500)
ORDER BY
    risk_score DESC,
    amount DESC
LIMIT 100
...
-- Можно заставить использовать именно idx_by_transaction_id явно
-- Например расширить саму проекцию нужными колонками, а не оставлять
...
-- сначала удалим не рабочую проекцию, она занимает место
ALTER TABLE financial_transactions DROP PROJECTION idx_by_transaction_id;
ALTER TABLE financial_transactions
ADD PROJECTION idx_by_transaction_id
(
...
В этом разделе рассмотрим skip-индексы как дополнительный способ оптимизации фильтрации и на практике проверим их эффективность для разных сценариев.
...
`ORDER BY` таблицы и неравномерным распределением значений, значительные пропуски. 
Тогда индекс быстро отсекает гранулы, где нужного значения заведомо нет.

Если бы индексируемая колонка была случайной (без корреляции с `ORDER BY`),
значения оказались бы размазаны по всем гранулам, и индекс пропуска почти не давал
бы выигрыша.
...
-- Сначала удалим конкурирующую проекцию
ALTER TABLE financial_transactions
    DROP PROJECTION idx_by_transaction_id;
-- и создадим индекс
ALTER TABLE financial_transactions
...
-- Важно: в колоночных СУБД и в BI практиках UNION часто является хорошей
-- альтернативой JOION
...
Не все данные, необходимые для аналитики, обязательно хранятся в основной широкой таблице.

В этом разделе рассмотрим, как обогащать ее справочными данными с помощью `JOIN` и словарей.
...
Типовой сценарий - дообогащение данных семантикой используется и на широких таблицах.
...
До этого мы проектировали и оптимизировали базовую таблицу. Теперь перейдем к подготовке производных показателей для использования в DataLens.

В этом разделе создадим представление с оконными функциями и проверим стоимость таких вычислений.
...
Если рассчитывать производные показатели при каждом обращении к `VIEW` слишком затратно, результаты можно подготовить заранее.

Для реализации классического подхода с таблицами-агрегатами в Clickhouse можно 
использовать материализованные представления и обновляемые материализованные представления.
...
- `avg_ms=5473`, `max_memory=2.09 GiB`, `avg_rows_read=50.00 million` — запрос из 5.2 (view + несколько оконных функций)
